...
class AcnooKotController extends Controller
{
    ...
    {
        $this->middleware('check.permission:kot.view')->only('index');
        $this->middleware('check.permission:kot.update')->only('cookingStatus', 'kotStatus');
    }
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::kots.datas', compact('kots'))->render(),
                'counts' => $kotCounts
            ]);
        }

        return view('restaurantwebaddon::kots.index', compact('kots', 'kotCounts', 'cancel_reasons'));
    }
    ...
    public function getKotTicket(string $id)
    {
        ...
        return view('restaurantwebaddon::kots.ticket', compact('kot'));
    }
}
