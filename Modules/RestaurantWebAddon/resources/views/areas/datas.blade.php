@foreach($areas as $area)
    <tr>
        ...
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item multi-delete" value="{{ $area->id }}">
        </td>
        @endusercan
        <td class="text-start">{{ ($areas->currentPage() - 1) * $areas->perPage() + $loop->iteration }}</td>
        <td class="text-start">{{ $area->name }}</td>
        <td class="text-start">{{ $area->total_table }}</td>
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                @usercan('areas.update')
                 <a title="{{__('Edit')}}" data-bs-placement="top" href="#areas-edit-modal" data-bs-toggle="modal" class="areas-edit-btn action-btn edit"
                        data-url="{{ route('business.areas.update', $area->id) }}"
                        data-name="{{ $area->name }}">
                    ...
                    @endusercan
                    @usercan('areas.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.areas.destroy', $area->id) }}" class="confirm-action action-btn delete" data-method="DELETE">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
