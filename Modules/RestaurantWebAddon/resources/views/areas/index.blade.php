...
<div class="erp-table-section">
    <div class="container-fluid">
        <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
            ...
            <form action="{{ route('business.areas.filter') }}" method="post" class="filter-form" table="#areas-data">
                @csrf
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    <div class="dropdown custom-dropdown">
                        ...
                        <div class="dropdown-content custom-dropdown-content">
                            <a href="{{ route('business.areas.pdf') }}" class="custom-item">
                                {{ __('PDF') }} </a>
                            <a href="{{ route('business.areas.csv') }}" class="custom-item">
                                {{ __('CSV') }} </a>
                            <a href="{{ route('business.areas.excel') }}" class="custom-item">
                                {{ __('EXCEL') }} </a>
                        </div>
                    </div>
                    @usercan('areas.create')
                        <a type="button" href="#areas-create-modal" data-bs-toggle="modal"
                ...
                <div class="delete-item delete-show multi-delete-container d-none mb-3">
                    <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                        ...
                        <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal"
                            data-url="{{ route('business.areas.delete-all') }}">{{ __('Delete') }}</button>
                    </div>
                </div>
                @endusercan
            </div>
                ...
                <table class="table" id="datatable">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start">{{ __('Tables') }}</th>
                            <th class="text-end">{{ __('Action') }}</th>
                        </tr>
                    </thead>
                    <tbody id="areas-data">
                        @include('restaurantwebaddon::areas.datas')
                    </tbody>
                </table>
            </div>
            <div class="mt-3">
...
@push('modal')
    ...
    @include('restaurantwebaddon::areas.create')
    @include('restaurantwebaddon::areas.edit')
@endpush
