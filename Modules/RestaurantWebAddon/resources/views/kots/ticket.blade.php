@extends('restaurantwebaddon::layouts.kitchen.blank')
...
@section('main_content')
    ...
    <div class="invoice-content invoice-content-size">
        ...
        <h2 class="text-center logo-title pt-2">{{ $kot->business->companyName ?? '' }}</h2>
        <h3 class="text-center kot-title">{{__('Order')}} : {{ $kot->invoiceNumber }}</h3>
        <div class="mb-3">
            <p class="text-black kot-info">{{__('Table No')}} : {{ $kot->table->name ?? '' }}</p>
            <p class="text-black kot-info">{{__('Date')}} : {{ formatted_date($kot->saleDate ?? '') }}  {{ formatted_time($kot->saleDate ?? '') }}</p>
        </div>
        <!-- Table -->
        <table class="ph-invoice-table">
            ...
            @php
                $totalItems = $kot->details->count() ?? 0;
                $totalQty = $kot->details->sum('quantities') ?? 0;
            @endphp
            <tbody>
                @foreach ($kot->details ?? [] as $detail)
                <tr>
