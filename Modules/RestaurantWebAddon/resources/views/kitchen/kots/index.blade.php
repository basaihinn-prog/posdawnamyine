@extends('restaurantwebaddon::layouts.kitchen.master')
...
<div class="erp-table-section">
        <div class="container-fluid">
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                ...
                <form action="{{ route('kitchen.kots.index') }}" method="Get" class="ajax-filter-form" table="#kots-data">
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        <a href="{{ route('kitchen.kots.index', array_merge(request()->except('status'), ['status' => 'pending'])) }}" class="filter-btn {{ request('status') == 'pending' ? 'active' : '' }}">{{__('Pending')}} (<span class="count-pending">{{ $kotCounts['pending'] ?? 0 }}</span>)</a>
                        <a href="{{ route('kitchen.kots.index', array_merge(request()->except('status'), ['status' => 'preparing'])) }}" class="filter-btn {{ request('status') == 'preparing' ? 'active' : '' }}">{{__('Preparing')}} (<span class="count-preparing">{{ $kotCounts['preparing'] ?? 0 }}</span>)</a>
                        <a href="{{ route('kitchen.kots.index', array_merge(request()->except('status'), ['status' => 'ready'])) }}" class="filter-btn {{ request('status') == 'ready' ? 'active' : '' }}">{{__('Ready')}} (<span class="count-ready">{{ $kotCounts['ready'] ?? 0 }}</span>)</a>
                        <a href="{{ route('kitchen.kots.index', array_merge(request()->except('status'), ['status' => 'cancelled'])) }}" class="filter-btn {{ request('status') == 'cancelled' ? 'active' : '' }}">{{__('Cancelled')}} (<span class="count-cancelled">{{ $kotCounts['cancelled'] ?? 0 }}</span>)</a>
            ...
            <div id="kots-data">
                @include('restaurantwebaddon::kitchen.kots.datas')
            </div>
        </div>
    </div>
@endsection
...
@push('modal')
    @include('restaurantwebaddon::kitchen.kots.cancel')
@endpush
