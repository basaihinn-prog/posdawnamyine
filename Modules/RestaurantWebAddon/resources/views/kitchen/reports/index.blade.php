        ...
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2><a href=""> {{__('Dashboard')}} </a>
                    <span>/ {{ __('Kot Reports') }} </span>
                </h2>
                <form action="{{ route('kitchen.kot-reports.index') }}" class="ajax-filter-form" method='GET' table="#kot-reports-data">
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="gpt-up-down-arrow position-relative">
                            <select name="per_page" class="custom-select">
                                <option @selected(request('per_page') == 10) value="20">{{ __('Show- 10') }}</option>
                                <option @selected(request('per_page') == 25) value="50">{{ __('Show- 25') }}</option>
                                <option @selected(request('per_page') == 50) value="100">{{ __('Show- 50') }}</option>
                                <option @selected(request('per_page') == 100) value="500">{{ __('Show- 100') }}</option>
                            </select>
                            <span></span>
                        </div>
                        ...
                        <div class="gpt-up-down-arrow position-relative">
                            <select name="status" class="custom-select">
                                ...
                                <option value="served" {{ request('status') == 'served' ? 'selected' : '' }}>{{ __('Served') }}</option>
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('kitchen.kot-reports.pdf', request()->all()) }}" class="custom-item export-btn"> {{ __('PDF') }} </a>
                                <a href="{{ route('kitchen.kot-reports.csv', request()->all()) }}" class="custom-item export-btn"> {{ __('CSV') }} </a>
                                <a href="{{ route('kitchen.kot-reports.excel', request()->all()) }}" class="custom-item export-btn"> {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                    </div>
                </form>
            ...
            <div class="card" id="kot-reports-data">
                @include('restaurantwebaddon::kitchen.reports.datas')
            </div>
...
@push('modal')
    @include('restaurantwebaddon::kitchen.reports.view')
@endpush
