@foreach($tables as $table)
    <tr>
        ...
        @endusercan
        <td  class="text-start">{{ ($tables->currentPage() - 1) * $tables->perPage() + $loop->iteration }}</td>
        <td class="text-start">{{ $table->name }}</td>
        <td class="text-start">{{ $table->area?->name }}</td>
        <td class="text-start">{{ $table->capacity }}</td>
        @if(moduleCheck('RestaurantOnlineStore'))
        <td class="text-start">
            <a href="{{ route('business.tables.show', $table->id) }}">
                <img src="{{ asset('assets/images/icons/qr_icone.svg') }}">
            </a>
        </td>
        @endif
