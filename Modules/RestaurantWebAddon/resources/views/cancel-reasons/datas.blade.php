@foreach ($cancel_reasons as $reason)
    <tr>
        @usercan('cancelReason.delete')
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item multi-delete" value="{{ $reason->id }}">
        </td>
        @endusercan
        <td class="text-start">{{ ($cancel_reasons->currentPage() - 1) * $cancel_reasons->perPage() + $loop->iteration }}</td>
        <td class="text-start">{{ ucfirst($reason->type) }}</td>
        <td class="text-start">{{ $reason->reason }}</td>
        <td>
            @usercan('cancelReason.update')
            <label class="switch">
                <input type="checkbox" {{ $reason->status == 1 ? 'checked' : '' }} class="status" data-url="{{ route('business.cancel-reasons.status', $reason->id) }}">
        ...
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    @usercan('cancelReason.update')
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="#cancel-reason-edit-modal" class="cancel-reason-edit-btn action-btn edit" data-bs-toggle="modal"
                        data-url="{{ route('business.cancel-reasons.update', $reason->id) }}" data-reason="{{ $reason->reason }}">
                    ...
                    @endusercan
                    @usercan('cancelReason.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.cancel-reasons.destroy', $reason->id) }}"
