@foreach($categories as $category)
    <tr class="table-content">
        ...
        <td class="table-single-content text-start">{{ $categories->perPage() * ($categories->currentPage() - 1) + $loop->iteration }}</td>

        <td class="text-center table-single-content">{{ $category->name }}</td>
        <td class="text-center table-single-content">{{ Str::limit($category->description, 35, '...') }}</td>
