...
trait HasUploader
{
    ...
    private function saveQrCode($qrContent, $disk = null, $oldFile = null)
    {
        $filename = now()->timestamp . '-' . rand(1, 1000) . '.svg';
        $path = 'uploads/qr_codes/' . date('y') . '/' . date('m') . '/';
        $filePath = $path . $filename;
        ...
        Storage::disk($disk ?? config('filesystems.default'))->put($filePath, $qrContent);
